...
namespace TestsXb.Db
{
    ...
    public class MsSqlColumnTests : MsSqlBase, IDisposable
    {
        ...
        public void ColumnTest()
        {
            ...
            var col = this._testModel.GetCol("COL_STR");
            ...
            Assert.IsTrue(col.IsAllowMultiByte);
            Assert.AreEqual(Encoding.GetEncoding("Shift_JIS"), col.Encoding);
            Assert.AreEqual(Xb.Db.Model.ErrorType.NoError, col.Error);

            col = this._test3Model.GetCol("COL_STR");
            Assert.AreEqual("COL_STR", col.Name);
            Assert.AreEqual(10, col.MaxLength);
            Assert.AreEqual(-1, col.MaxInteger); //文字列なので整数桁値は存在しない -> -1
            Assert.AreEqual(-1, col.MaxDecimal); //文字列なので少数桁値は存在しない -> -1
            ...
            col = this._test3Model.GetCol("COL_INT");   //2147483647
            ...
            Assert.IsTrue(col.IsPrimaryKey);
            Assert.IsFalse(col.IsNullable);
            Assert.IsFalse(col.IsAllowMultiByte);
            Assert.AreEqual(Encoding.GetEncoding("Shift_JIS"), col.Encoding);
            Assert.AreEqual(Xb.Db.Model.ErrorType.NoError, col.Error);
        ...
        public void ValidateTest()
        {
            ...
            col = this._testModel.GetCol("COL_STR");
            ...
            col.IsAllowMultiByte = false;
            Assert.AreEqual(Xb.Db.Model.ErrorType.DetectMultiByteChar, col.Validate("１２３４５"));
            ...
            Assert.AreEqual(Xb.Db.Model.ErrorType.DecimalOver, col.Validate(-12.3456));
            Assert.AreEqual(Xb.Db.Model.ErrorType.NotNumeric, col.Validate("a"));
            Assert.AreEqual(Xb.Db.Model.ErrorType.NoError, col.Validate(null));
            col = this._test3Model.GetCol("COL_INT");
            Assert.AreEqual(Xb.Db.Model.ErrorType.NoError, col.Validate(12));
            Assert.AreEqual(Xb.Db.Model.ErrorType.NoError, col.Validate(-12));
            ...
            col = this._test3Model.GetCol("COL_DATETIME");
