...
namespace Xb.Db
{
    ...
    public partial class Model : IDisposable
    {
        ...
        private DataTable _newRowBaseDataTable;
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public List<Db.Model.Column> Columns => this._columns.ToList();
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public List<Db.Model.Column> PkeyColumns => this._pkeyColumns.ToList();
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public DataTable TableInfo => this._tableInfo;
        ...
        public Model(Db.DbBase db, DataTable tableInfo)
        {
            if (tableInfo == null 
                || tableInfo.Rows.Count <= 0 
                || tableInfo.Columns.Count <= 0)
            {
                Xb.Util.Out("Db.Model.New: カラム情報DataTable、もしくはテーブル名が検出出来ません。");
                throw new ArgumentException("カラム情報DataTable、もしくはテーブル名が検出出来ません。");
            }
            ...
            //テーブル名を取得する。
            this._tableName = tableInfo.Rows[0]["TABLE_NAME"].ToString();

            //カラム個数分の配列を用意する。
            this._columns = new Xb.Db.Model.Column[tableInfo.Rows.Count];

            List<Xb.Db.Model.Column> pkeyColumns = new List<Xb.Db.Model.Column>();

            //専用DataRow生成用DataTableを初期化
            this._newRowBaseDataTable = new DataTable();
            this._newRowBaseDataTable.TableName = this._tableName;

            //カラム個数分ループ
            ColumnType type = default(ColumnType);
            int maxInteger = 0;
            int maxDecimal = 0;
            int maxLength = 0;
            for (int i = 0; i <= tableInfo.Rows.Count - 1; i++)
            {
                typeString = tableInfo.Rows[i]["TYPE"].ToString().ToUpper();

                if (_typesOfNumber.Contains(typeString))
                {
                    type = ColumnType.Number;
                    maxInteger = int.Parse(tableInfo.Rows[i]["NUM_PREC"].ToString());
                    maxDecimal = int.Parse(tableInfo.Rows[i]["NUM_SCALE"].ToString());
                ...
                else if (_typesOfString.Contains(typeString))
                {
                    ...
                    if (!int.TryParse(tableInfo.Rows[i]["CHAR_LENGTH"].ToString(), out maxLength))
                    {
                ...
                isPkey = (tableInfo.Rows[i]["IS_PRIMARY_KEY"].ToString() == "1");

                //DataTable上の型都合で、"1"と"true"と2種類取れてしまうため、整形する。
                var nullable = (tableInfo.Rows[i]["IS_NULLABLE"].ToString().ToLower().Replace("true", "1") == "1");

                this._columns[i] = new Xb.Db.Model.Column(tableInfo.Rows[i]["COLUMN_NAME"].ToString(), 
                                                          maxLength, 
                                                          maxInteger, 
                                                          maxDecimal, 
                                                          type, 
                ...
                //専用DataRow生成用DataTableへカラムを追加。
                //カラムは全て文字列型。
                this._newRowBaseDataTable.Columns.Add(this._columns[i].Name, typeof(string));
            }
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public DataRow Find(object primaryKeyValue)
        {
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public DataRow Find(params object[] primaryKeyValues)
        {
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public virtual DataTable FindAll(string whereString = null, string orderString = null)
        {
        ...
        /// <summary>
        /// モデル用CRUD処理専用DataRowを生成する。
        /// </summary>
        /// <returns></returns>
        /// <remarks></remarks>
        public DataRow NewRow()
        {
            return this._newRowBaseDataTable.NewRow();
        }
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public Xb.Db.Model.Error[] Validate(DataRow row)
        {
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public Xb.Db.Model.Error[] Write(DataRow row, params string[] excludeColumnsOnUpdate)
        {
            ...
            {
                var value = row.Table.Columns.Contains(col.Name)
        ...
        /// <param name="row"></param>
        /// <returns></returns>
        public Xb.Db.Model.Error[] Insert(DataRow row)
        {
            var colNames = new List<string>();
            foreach (DataColumn col in row.Table.Columns)
                colNames.Add(col.ColumnName);
            ...
            foreach (Xb.Db.Model.Column col in this._columns)
                colValues.Add(col.Name, 
                              row.Table.Columns.Contains(col.Name)
                                  ? this.NullFormat(row[col.Name])
                                  : null);
        ...
        /// <returns></returns>
        public Db.Model.Error[] Update(DataRow row, string[] keyColumns = null, string[] excludeColumns = null)
        {
            var colNames = new List<string>();
            foreach (DataColumn col in row.Table.Columns)
                colNames.Add(col.ColumnName);
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public Xb.Db.Model.Error[] Delete(DataRow row, params string[] keyColumns)
        {
            if (keyColumns == null
                || keyColumns.Length == 0)
                keyColumns = this._pkeyColumns.Select(col => col.Name).ToArray();
            ...
            foreach (string col in keyColumns)
            {
                wheres.Add(this.GetCol(col).GetSqlFormula(row[col]));
            }
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public Xb.Db.Model.Error[] ReplaceUpdate(DataRow[] drsAfter, 
                                                 DataRow[] drsBefore = null,
                                                 params string[] excludeColumnsOnUpdate)
        {
            ...
            foreach (DataRow row in drsAfter)
            {
            ...
            {
                //旧DataRow配列の全行ループ
                foreach (DataRow rowBefore in drsBefore)
                {
                    var isDeleteTarget = true;

                    //新DataRow配列の全行ループ
                    foreach (DataRow rowAfter in drsAfter)
                    {
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public Xb.Db.Model.Error[] ReplaceUpdate(DataTable dtAfter, 
                                                 DataTable dtBefore = null, 
                                                 params string[] excludeColumnsOnUpdate)
        {
            return this.ReplaceUpdate(dtAfter.AsEnumerable().Select(row => row).ToArray(), 
                                      dtBefore.AsEnumerable().Select(row => row).ToArray(), 
        ...
        protected virtual void Dispose(bool disposing)
        {
            lock (this)
            {
                if (disposing)
                {
                    ...
                    this._newRowBaseDataTable = null;
